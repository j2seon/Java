...
public class BankAccountUsiningArrayList {
	
	//컬랙션(ArrayList<E>)를 사용해서 Account 객체생성
	private static ArrayList<Account> arraylist = new ArrayList<>(); 	//배열에 객체 저장. 배열은 방의 크기를 정해줘야한다.
		//배열은 방의 크기가 고정, 선언시 방의 크기를 지정, 방의 크기를 수정할 수 없다.
		//컬렉션은 방의 크기가 동적 무한정 저장
		//값을 추가할때 add(): 개수:size(); 값을 삭제시 remove() , ArrayList는 null이 없다.
	...
	private static void  accountList() {
		...
		for(int i=0; i<arraylist.size(); i++) {
			...
			System.out.println("계좌번호 : "+arraylist.get(i).getAno());
			System.out.println("계좌주 : "+arraylist.get(i).getOwnew());
			System.out.println("금액 : "+ arraylist.get(i).getBalance());
			System.out.println();
			index++;
...
//		System.out.println(findAccount(ano).getBalance());
	...
	private static Account findAccount(String ano) {
		...
		for(int i=0; i<arraylist.size(); i++) {
			if(arraylist.get(i).getAno().equals(ano)) {
				account=arraylist.get(i); //객체 주소를 담음  
			}
		}
		return account;
	}
